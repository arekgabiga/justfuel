            ...
            if (table === 'fillups') {
                ...
                // Implementation for UPDATE
                 chain.update.mockImplementation((data: any) => {
                    return {
                        eq: (col: string, val: any) => { // id match
                ...
                const queryChain = {
                    select: () => queryChain,
                    eq: (col: string, val: any) => { filters[col] = val; return queryChain; },
                    neq: (col: string, val: any) => { filters[`neq_${col}`] = val; return queryChain; },
                    lt: (col: string, val: any) => { filters[`lt_${col}`] = val; return queryChain; },
                    gt: (col: string, val: any) => { filters[`gt_${col}`] = val; return queryChain; },
                    gte: (col: string, val: any) => { filters[`gte_${col}`] = val; return queryChain; },
                    order: (col: string, dir: any) => { filters['order'] = { col, dir }; return queryChain; },
                    limit: (val: number) => { filters['limit'] = val; return queryChain; },
                    maybeSingle: async () => {
